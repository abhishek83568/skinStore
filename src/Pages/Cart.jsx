...
const Cart = () => {
  ...
  const calculateTotal = () => {
    return data.reduce((total, item) => total + parseFloat(item.price.replace('$', '')) * item.Qty, 0).toFixed(2);
  };
      ...
      {data.length === 0 ? (
        ...
        <table className='cart-table'>
          ...
          <tbody>
            {data.map(item => (
              <tr key={item.id}>
                <td><img src={item.image_path} alt={item.title} className='cart-item-image' /></td>
                <td>{item.title}</td>
                <td>{item.price}</td>
                <td>
                  <input 
